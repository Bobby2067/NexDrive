...
**Change Note (v2.0):** Provider updated from Vapi.ai to Retell AI — reflects actual codebase state. Added §0 Claude Code Navigation to prevent directory confusion. All Vapi references replaced with Retell equivalents. Adapter pattern preserved — business logic unchanged.  
...
## 0. Claude Code Navigation (Read First)

> **This section is for AI coding agents (Claude Code, Cursor, etc.) — not end users.**  
> Ignore this section when reading the spec for conceptual understanding.

### 0.1 Critical: Working Directory

The entire Next.js application lives inside a subdirectory called `nexdrive-app`. **404 errors on API routes and `module not found` errors are almost always caused by running commands from the wrong directory.**

Before running any `npm` command, reading any source file, or analysing any API route, verify you are operating from:

```
C:/Users/RobOgilvie/OneDrive - Ion Dna pty Ltd (1)/01_ACTIVE_PROJECTS/NexDrive Academy/cLAUDE DESIGN/nexdrive-app
```

**Always `cd nexdrive-app` first.** Do not attempt to run `npm run dev` or access `src/` from the parent directory — it does not contain a Next.js app.

### 0.2 Existing Files to Read Before Writing Code

The following files already exist in the codebase. Read them before making any changes:

| File | Why |
|------|-----|
| `nexdrive-app/src/lib/voice/provider.interface.ts` | Defines the standard shape of a voice provider — read this first to understand the contract |
| `nexdrive-app/src/lib/voice/retell.adapter.ts` | Retell AI implementation of the provider interface — understand the existing approach before extending |
| `nexdrive-app/src/lib/voice/functions/` | Functions Factory — definitions and logic for custom tools the LLM can call |
| `nexdrive-app/src/app/api/v1/voice/inbound/route.ts` | Inbound call webhook handler (Retell → NexDrive) |
| `nexdrive-app/src/app/api/v1/voice/event/route.ts` | Call event webhook handler (status updates, transcripts) |
| `nexdrive-app/src/app/api/v1/voice/function-call/route.ts` | Function call dispatcher route (custom tool execution) |
| `nexdrive-app/src/db/index.ts` | Database connection (Drizzle + Neon) |
| `nexdrive-app/src/db/schema/` | All table definitions |

### 0.3 Required Environment Variables

All voice agent env vars live in `nexdrive-app/.env` (or `.env.local`). The two you need before anything works:
...
Full env var list (including webhook secret, voice ID, agent ID, etc.) in §16.

### 0.4 Next Steps for Claude Code

Suggested onboarding order when picking up this component fresh:

1. **Verify working directory** — confirm you are inside `nexdrive-app` (absolute path in §0.1). Run `pwd` or `ls src/` to confirm before touching anything.
2. **Read the adapter** — open `src/lib/voice/retell.adapter.ts` to understand the current Retell implementation before extending or modifying it.
3. **Read the API routes** — open `src/app/api/v1/voice/inbound/route.ts`, `event/route.ts`, and `function-call/route.ts` to verify how the adapter is consumed by the Next.js endpoints before adding new behaviour.
...
1. **Provider-agnostic via adapter pattern.** Retell AI is the current implementation, but all voice provider logic goes through a `VoiceAgentProvider` interface (per ADR-008). Provider can be swapped without touching business logic.
...
8. **Webhook security.** All inbound webhook endpoints verify the Retell webhook signature (`x-retell-signature` HMAC-SHA256). No unauthenticated processing.
...
Inbound Call (NexDrive Business Number)
     ...
     ▼
┌──────────────────────────┐
│  Retell AI Receives Call  │  ← Provider handles STT, TTS, turn-taking
...
│   │   ├── retell.adapter.ts                 # Retell AI implementation of provider interface ← EXISTS
│   │   ├── retell.webhook-verify.ts          # Retell webhook signature verification (HMAC-SHA256)
│   │   ├── retell.agent-config.ts            # Retell agent + LLM definition (system prompt, functions)
...
│           └── voice/
│               ├── inbound/
│               │   └── route.ts              # POST /api/v1/voice/inbound ← EXISTS
│               ├── event/
│               │   └── route.ts              # POST /api/v1/voice/event ← EXISTS
│               └── function-call/
│                   └── route.ts              # POST /api/v1/voice/function-call ← EXISTS
│
└── __tests__/
    └── voice/
        ├── retell.adapter.test.ts
...
```typescript
// nexdrive-app/src/lib/voice/index.ts
...
const providers: Record<string, () => VoiceAgentProvider> = {
  ...
  // vapi: () => new VapiAdapter(),   // future / evaluation
  // bland: () => new BlandAdapter(), // future / evaluation
};
...
export function getVoiceProvider(): VoiceAgentProvider {
  ...
  const providerId = process.env.VOICE_PROVIDER ?? 'retell';
...
## 4. Retell AI Adapter (`retell.adapter.ts`)


**This file already exists at `nexdrive-app/src/lib/voice/retell.adapter.ts`. Read it before making changes.**
...
import { verifyRetellWebhook } from './retell.webhook-verify';
import { RETELL_API_BASE } from './constants';
 ...
 *
 * Authentication: Bearer token via RETELL_API_KEY env var.
 * All API calls to: https://api.retellai.com
 *
 * Key Retell concepts:
 * - "Agent" = the configured AI voice persona (equivalent to Vapi "assistant")
 * - "LLM" = a separate Retell object defining the model, prompt, and tools
 *   (agent references an LLM by ID via RETELL_LLM_ID)
 * - Webhooks use x-retell-signature HMAC-SHA256 header
 * - Call ID is at payload.call.call_id (not payload.call.id)
 */
export class RetellAdapter implements VoiceAgentProvider {
  ...
  constructor() {
    ...
    if (!this.apiKey) throw new Error('RETELL_API_KEY environment variable is required');
    this.baseUrl = RETELL_API_BASE;
  }
  ...
  private async fetch(path: string, options: RequestInit = {}): Promise<Response> {
    ...
    if (!res.ok) {
      ...
      throw new VoiceProviderError(
        `Retell API error: ${res.status} ${res.statusText}`,
  ...
  async createAssistant(config: AssistantConfig): Promise<{ assistantId: string }> {
    // In Retell, we create/update via the agent endpoint.
    // The LLM object (prompt + tools) must be created separately in the dashboard
    // or via POST /create-retell-llm, then referenced by RETELL_LLM_ID.
    const retellPayload = this.toRetellAgentPayload(config);
    const res = await this.fetch('/v2/create-agent', {
      method: 'POST',
  ...
  async updateAssistant(agentId: string, config: Partial<AssistantConfig>): Promise<void> {
    const retellPayload = this.toRetellAgentPayload(config as AssistantConfig);
    await this.fetch(`/v2/update-agent/${agentId}`, {
      method: 'PATCH',
      body: JSON.stringify(retellPayload),
    });
  }
  ...
  async getCallStatus(callId: string): Promise<CallStatus> {
    const res = await this.fetch(`/v2/get-call/${callId}`);
    const data = await res.json() as RetellCallResponse;

    return {
      callId: data.call_id,
      status: this.mapRetellCallStatus(data.call_status),
      duration_seconds: data.end_timestamp && data.start_timestamp
        ? Math.round((data.end_timestamp - data.start_timestamp) / 1000)
        : undefined,
      callerPhone: data.from_number,
      startedAt: data.start_timestamp ? new Date(data.start_timestamp) : undefined,
      endedAt: data.end_timestamp ? new Date(data.end_timestamp) : undefined,
    };
  }

  async endCall(callId: string): Promise<void> {
    await this.fetch(`/v2/end-call/${callId}`, { method: 'POST' });
  }
  ...
  async verifyWebhook(
    ...
    return verifyRetellWebhook(payload, headers, process.env.RETELL_WEBHOOK_SECRET!);
  }

  parseCallEndedEvent(rawEvent: Record<string, unknown>): CallEndedEvent {
    // Retell sends the call object directly at the top level of the webhook payload
    const call = rawEvent.call as RetellCallResponse;
    ...
    return {
      ...
      endReason: this.mapRetellDisconnectReason(call.disconnection_reason),
      recordingUrl: call.recording_url,
    };
  }

  parseFunctionCallRequest(rawRequest: Record<string, unknown>): FunctionCallRequest {
    // Retell function-call webhook shape:
    // { call: RetellCallResponse, name: string, arguments: Record<string, unknown> }
    const call = rawRequest.call as RetellCallResponse;

    return {
      callId: call.call_id,
      functionName: rawRequest.name as string,
      parameters: (rawRequest.arguments as Record<string, unknown>) ?? {},
      callerPhone: call.from_number ?? 'unknown',
    };
  }

  formatFunctionCallResponse(response: FunctionCallResponse): Record<string, unknown> {
    // Retell expects the function result as a plain string
    return {
  ...
  private toRetellAgentPayload(config: AssistantConfig): Record<string, unknown> {
    return {
      agent_name: config.name,
      llm_websocket_url: undefined,              // Not used — we use Retell-hosted LLM
      retell_llm_dynamic_variables: {},
      voice_id: config.voiceId,                  // ElevenLabs voice ID from RETELL_VOICE_ID
      language: 'en-AU',
      response_engine: {
        type: 'retell-llm',
        llm_id: process.env.RETELL_LLM_ID,       // Created separately in Retell dashboard
      },
      webhook_url: config.webhooks.eventUrl,
      begin_message: NEXDRIVE_GREETING,
      end_call_after_silence_ms: 30_000,
      max_call_duration_ms: 600_000,             // 10 minutes
  ...
  private mapRetellCallStatus(status: string): CallStatus['status'] {
  ...
  private mapRetellDisconnectReason(reason?: string): CallEndedEvent['endReason'] {
    const map: Record<string, CallEndedEvent['endReason']> = {
      'user_hangup': 'caller_hangup',
      'agent_hangup': 'assistant_hangup',
      'call_transfer': 'assistant_hangup',
      'silence_timeout': 'timeout',
      'max_duration_reached': 'timeout',
      'error_infer': 'error',
      'error_llm_websocket_open': 'error',
      'error_llm_websocket_lost_connection': 'error',
    };
...
// --- Retell-specific types (internal, not exported) ---
...
### 4.2 Webhook Signature Verification (`retell.webhook-verify.ts`)
 ...
 *
 * Retell signs all webhook requests with HMAC-SHA256 using the webhook
 * signing secret from the Retell dashboard. The signature is sent in the
 * `x-retell-signature` header.
 *
 * Verification: HMAC-SHA256(raw_body, RETELL_WEBHOOK_SECRET) === x-retell-signature
 *
 * If Retell changes their verification method, only this file needs updating.
 */
export async function verifyRetellWebhook(
  ...
  if (!signature) {
    throw new WebhookVerificationError('Missing x-retell-signature header');
  }
  ...
  if (!isValid) {
    throw new WebhookVerificationError('Invalid Retell webhook signature');
  }
...
## 5. Retell Agent Configuration (`retell.agent-config.ts`)

This file defines the NexDrive AI receptionist — its persona, knowledge, and capabilities. It is deployed to Retell via `createAssistant()` during setup, and updated via `updateAssistant()` whenever business info changes.

**Note on Retell architecture:** The system prompt and tool definitions live in a **Retell LLM object** (created separately in the Retell dashboard or via their API). The agent configuration references the LLM object by `RETELL_LLM_ID`. This file builds the agent-level config; the LLM object is managed separately.
...
/**
 * NexDrive AI Receptionist — Retell Agent Configuration
 *
 * This is deployed to Retell via createAssistant() during setup.
...
export function buildAssistantConfig(): AssistantConfig {
  return {
    ...
    voiceId: process.env.RETELL_VOICE_ID ?? 'default-australian-female',
    language: 'en-AU',
    ...
    providerOptions: {
      // Retell-specific tuning
      end_call_after_silence_ms: 30_000,
      max_call_duration_ms: 600_000,
      enable_backchannel: true,
      ambient_sound: 'off',
      responsiveness: 0.9,               // Retell scale: 0-1, higher = faster response
    },
  };
}
...
The dispatcher receives function calls from the voice agent and routes them to the appropriate handler. Each handler is a thin wrapper that calls existing service-layer APIs (Booking Engine, RAG Engine, CRM).
 ...
 *
 * Called by Retell AI when an inbound call starts.
 * This is a webhook endpoint — no Clerk auth, verified by Retell signature.
 *
 * Flow:
 * 1. Verify webhook signature (x-retell-signature)
 ...
 * 7. Return 200 immediately (Retell expects fast response)
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const messageType = (payload.message as any)?.type ?? payload.type ?? payload.event;

    // Only process 'call_started' events (Retell event type)
    if (messageType !== 'call_started') {
      return NextResponse.json({ ok: true });
    }

    const call = (payload.message as any)?.call ?? payload.call ?? {};
    const callId = (call.call_id ?? call.id) as string;
    const callerPhone = (call.from_number ?? call.customer?.number) as string ?? 'unknown';
    ...
    // Return 200 anyway — don't make Retell retry for our internal errors
    return NextResponse.json({ ok: true });
  }
}
```
 ...
 *
 * Called by Retell AI when a call event occurs (call ended, transcript ready, etc.).
 * This is a webhook endpoint — no Clerk auth, verified by Retell signature.
 *
 * Primary event: call_ended
 * Contains: full transcript, AI-generated summary, duration, disconnection reason.
 *
 * Flow:
 * 1. Verify webhook signature (x-retell-signature)
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Only process call_ended events (Retell event type)
 ...
 *
 * Called by Retell AI when the LLM wants to invoke a function (custom tool).
 ...
 * 6. Return result (Retell reads it aloud to the caller)
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
...
export const VOICE_PROVIDER_IDS = ['retell', 'vapi', 'bland'] as const;
...
Add to `nexdrive-app/.env.local` (and Vercel environment settings):

```bash
# Voice Agent — Retell AI
VOICE_PROVIDER=retell
RETELL_API_KEY=                        # Retell API key (from dashboard → API Keys)
RETELL_WEBHOOK_SECRET=                 # Webhook signing secret (from Retell dashboard → Webhooks)
RETELL_VOICE_ID=                       # ElevenLabs voice ID for Australian accent (configured in Retell)
RETELL_AGENT_ID=                       # Set after first deployment via createAssistant()
RETELL_LLM_ID=                         # Retell LLM object ID (created in dashboard, contains prompt + tools)
